				...
				if(nCount>=nCellIndex && bFinish==false)
					...
					if(nCount==nCellIndex)
						...
						if(oTD_tmp.colSpan==nColSpan)
							...
							if(bDeleteColumn)
								{
								alert("Cannot delete column. The column contains spanned cells from another column. Please remove the span first.");
								return;
								}						
		...
		for(var j=0;j<oTR_tmp.childNodes.length;j++)
			...
			if(oTD_tmp.rowSpan>=oTR.rowIndex+1-i)
				...
				for(var k=0;k<oTD_tmp.colSpan;k++)
					...
					if(bDeleteRow)
						{
						alert("Cannot delete row. The row contains spanned cells from another rows. Please remove the span first.")
						return;
						}				
					}
				}
