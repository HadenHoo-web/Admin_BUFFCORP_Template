	...
	if ($giaoviec_id == '0'){
		...
		if ((int)$member_id > 0) {
			...
			if (!empty($member['zalo_user_id'])) {
				...
				file_put_contents(
					dirname(__FILE__).'/../../zalo/logs/zalo_send.log',
					date('Y-m-d H:i:s')." | CREATE_TASK | ".json_encode($res, JSON_UNESCAPED_UNICODE).PHP_EOL,
					FILE_APPEND
				);
				...
				file_put_contents(
					dirname(__FILE__).'/../../zalo/logs/zalo_send.log',
					date('Y-m-d H:i:s')." | NO_ZALO_USER_ID | member_id={$member_id}".PHP_EOL,
					FILE_APPEND
				);
			}
		}
		...
		file_put_contents(
			dirname(__FILE__).'/../../zalo/logs/zalo_send.log',
			date('Y-m-d H:i:s')." | DONE_CHECK | old={$old_status} new={$new_status} ".
			"assignee={$rowOld['member_id']} actor_login={$actor_login_id} actor_member={$actor_member_id} ".
			"created_by_id={$rowOld['created_by_id']} should=".($shouldNotifyDone?1:0).PHP_EOL,
			FILE_APPEND
		);
		...
		if ($shouldNotifyDone) {
			...
			if (!empty($boss['zalo_user_id'])) {
				...
				file_put_contents(
					dirname(__FILE__).'/../../zalo/logs/zalo_send.log',
					date('Y-m-d H:i:s')." | DONE_NOTIFY | giaoviec_id=".$rowOld['giaoviec_id']." | ".json_encode($res, JSON_UNESCAPED_UNICODE).PHP_EOL,
					FILE_APPEND
				);
				...
				file_put_contents(
					dirname(__FILE__).'/../../zalo/logs/zalo_send.log',
					date('Y-m-d H:i:s')." | DONE_NOTIFY_NO_BOSS_ZALO | created_by_id=".$rowOld['created_by_id']." | giaoviec_id=".$rowOld['giaoviec_id'].PHP_EOL,
					FILE_APPEND
				);
			}
		}
