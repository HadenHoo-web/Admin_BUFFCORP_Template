...
//----------------------------------------------------------------------------------------------------------------------------------------	
    ...
    $note	    = mosGetParam( $_REQUEST, 'note', '');
		...
		{	$password = md5($item_name);	
			$sql = "insert into tbl_member (loginname, password, fullname, address, note, email, member_type_id, extra_member_type_id, phone, luong, cellphone, bhxh_code, bhxh_salary, bhxh_start_date, bhxh_status, bhxh_note, active, created_date, created_by,role_id) values ('$item_name', '$password', '$fullname', '$address', '$note', '$email', '$member_type_id', '$extra_member_type_id', '$phone', '$luong', '$cellphone', '$bhxh_code', '$bhxh_salary', ".($bhxh_start_date != '' ? "'$bhxh_start_date'" : "NULL").", '$bhxh_status', '$bhxh_note', $active, now(), '".$_SESSION['membername']."','$role')";
		} else
			$sql = "update tbl_member set loginname = '$item_name', fullname = '$fullname', address = '$address', note = '$note', email = '$email', member_type_id = '$member_type_id', extra_member_type_id = '$extra_member_type_id', phone = '$phone', luong = '$luong', cellphone = '$cellphone', bhxh_code = '$bhxh_code', bhxh_salary = '$bhxh_salary', bhxh_start_date = ".($bhxh_start_date != '' ? "'$bhxh_start_date'" : "NULL").", bhxh_status = '$bhxh_status', bhxh_note = '$bhxh_note', active = $active ,role_id='$role' where member_id = $item_id";
		if ( !($result = $db->sql_query($sql)) ) message_die( DATABASE_BUSY );
		mosList();
	}
//----------------------------------------------------------------------------------------------------------------------------------------
